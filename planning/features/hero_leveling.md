# Feature: Hero Leveling

- **Last Updated**: 2026-03-24
- **Status**: NOT STARTED
- **Pod**: Metagame
- **Design Owner**: Leonard Perez
- **ClickUp**: [TBD]
- **Notion**: [TBD]

---

## Why This Feature

### Validation Goals

| SHQ | Question | Status |
|-----|----------|--------|
| SHQ3-12 | Do players feel they have enough agency in upgrading their favorite characters? | NOT STARTED |

**Parent BHQ**: BHQ-M1 (hero collectability), BHQ-E3 (long-term progression)

**What Hero Leveling Must Prove**: XP-based leveling provides satisfying, incremental hero power growth with clear feedback.

### Success Criteria

- [TBD - needs designer input]

---

## Scope

System for leveling heroes through XP. Stat growth, level caps, XP sources.

### Core Mechanics

- [TBD - needs designer input]

### In Scope

- [TBD - needs designer input]

### Out of Scope

- [TBD - needs designer input]

---

## Estimate & Approach

**Total Estimate**: [TBD]

### Disciplines Required

| Discipline | Needed | Sprints | Notes |
|-----------|--------|---------|-------|
| Engineering | [TBD] | [TBD] | |
| UX/UI Design | [TBD] | [TBD] | |
| Game Design | [TBD] | [TBD] | |

### Implementation Flow

[TBD - needs engineering input]

### Pre-Conditions

- [ ] [TBD - needs designer input]

---

## Dependencies

| Dependency | Direction | Details |
|-----------|-----------|---------|
| Hero Info | Related | Level displayed on hero info screen |
| Governors | Related | Governor effectiveness tied to hero level |

---

## Risks

| Risk | Impact | Probability | Mitigation |
|------|--------|-------------|------------|
| [TBD] | | | |

---

## Open Questions

- [ ] [TBD - needs designer input]

---

## References

- Notion Design Doc: [TBD]
- ClickUp Epic: [TBD]
- Related: `planning/ValidationPlan.md`
